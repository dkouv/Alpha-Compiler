...
/******************************************************************/
struct argument_mias_sinartisis* insert_new_argument(struct argument_mias_sinartisis* head, char* arg_name){
    /* an to head einai null, initialize */
    if(head == NULL){
        head = calloc(sizeof(struct argument_mias_sinartisis), sizeof(struct argument_mias_sinartisis));
        head->name = strdup(arg_name);
        return head;
    }else {
        struct argument_mias_sinartisis* tmp, *node = calloc(sizeof(struct argument_mias_sinartisis), sizeof(struct argument_mias_sinartisis));
        node -> name = strdup(arg_name);

        tmp = head;
        /* vriskw to teleutaio stoixeio sti lista gia na kanw ti sindesi */
        while(tmp->next){
            tmp = tmp->next;
        }
        tmp->next = node;
...
expr create_new_temp_var(){
    ...
    result->strConst = strdup(create_name_with_num("_tmp", hidden_tmp_var_counter));
    //result->const_int = hidden_tmp_var_counter;
    //tmp_var->number = hidden_tmp_var_counter;

    if (scope == GLOBAL_T){
        SymTable_put(Symbol_Table, result->strConst, yylineno, scope, assign_counter, GLOBAL_T);
    }else{
        SymTable_put(Symbol_Table, result->strConst, yylineno, scope, assign_counter, LOCAL_T);
    }
...
expr calculate_expr2(int op, expr arg1, expr arg2){
    ...
    if(arg1->type != tableitem_e)
        ...
        create_quad(op, arg1, arg1, result);
