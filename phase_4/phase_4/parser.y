...
expr		:assignexpr							{printf(BLUE "assignexpr\n" RESET);}
			|expr PLUS expr 					
				{
					...
					printf("TIPOS :%s\n", expression_types[$1->type]);
					$$ = calculate_expr(add, $1, $3);
			...
			|expr AND expr 						{printf(BLUE "expr and expr\n" RESET);printf("str :%s %s\n", $1->strConst , $3->strConst); $$ = calculate_expr(and, $1, $3); $$->type = boolexpr_e;}
			|expr OR expr 						{printf(BLUE "expr or expr\n" RESET); $$ = calculate_expr(or, $1, $3); $$->type = boolexpr_e;}
			|term 								
			{
				only_term ++;
				printf(BLUE "term\n" RESET);
			...
			|lvalue MINUS_MINUS								
				{
					...
					create_quad(assign,  t, $lvalue, $lvalue);
					create_quad(sub, $lvalue, ena, $lvalue);
					$$ = t;
					printf(BLUE "lvalue-- -> " RESET);
				}
...
assignexpr 	:lvalue EQUAL expr								
				{	
					...
					if(expr_node){
						...
						if(symbol_get_value_type(expr_node->type) == 1 && strcmp($1->sym->value.varVal->name, expr_node->value.funcVal->name) == 0){
							printf(RED "Cannot perform assignment with function at line :%d\n"RESET, yylineno); 
						} 
					}else {
						entry e = look_up($1->sym->value.varVal->name, scope);
...
lvalue		:IDENTIFIER						
				...
				if (conflict_with_system_function($1) == 0){
					...
					$$->sym = expr_node;//Scope_List_Contains_Symbol($IDENTIFIER, scope);
					//$$->type = number_e;
				}else {
					printf("Referring to LIBFUNC:%s\n", $1);

					$$->sym = Scope_List_Contains_Symbol($IDENTIFIER, 0);
					//printf(RED"PTR :%p\n", $$->sym);
					$$->type = 	libraryfunc_e;
				}	
...
funcdefstmt	:FUNCTION IDENTIFIER 
		{
			...
			if(node){
				...
				if(conflict_with_system_function($2) == 1){
					...
					function_list_insert(Scope_List_Contains_Symbol($2, scope));
			...
			func_expr->sym = look_up($2, scope);
			printf("%p\n",func_expr->sym);
		
			create_quad(funcstart, func_expr, true_expression, true_expression);

		}	
		LEFT_PARENTHESES {enter_scope_space(FORMAL_T);}idlist {remove_last_scope_space();}RIGHT_PARENTHESES {enter_scope_space(functionLocal);eimai_se_function++;} block {eimai_se_function--;remove_last_scope_space();} 
			{
				printf("PARAMETROI\n");
				/************************************************************/
		...
		{enter_scope_space(2);}idlist {remove_last_scope_space();}RIGHT_PARENTHESES {enter_scope_space(functionLocal);eimai_se_function++;} block {eimai_se_function--;remove_last_scope_space();} 
				{
					...
					function_list_remove_last();
					se_ti_loop_eimai = 0;
					$funcdefstmt = get_function_entry(func_expr->strConst, scope);
