...
entry look_up(char *pv_value, int scope){
	...
	for(;scope_count != - 1; scope_count--){
		...
		if(found_scope_list_entry && found_scope_list_entry->isActive == 1){
...
int SymTable_put(SymTable_T oSymTable,  char *pv_value, int line, int scope, int assign_counter, int symbol_table_type) {
	...
	switch(symbol_table_type) {
		...
		case 3 :
			...
			printf(YELLOW"THELOUN RESET AN DEN FTIAXTEI\n"RESET);
	...
	if(space_scope == functionLocal){
		if(node->scope_offset != -1 && dont_increase_locals == 0)
...
void print_entry_node_information(entry node){
	...
	if(symbol_get_value_type(node->type) == 1){
			...
			printf("Function %s me scope :%d se grammi:%d me tipo :%s einai active:%d offset %d total_args :%d\n", p->name, p->scope, p->line, variable_types[node->type], node->isActive,   node->scope_offset, p->total_arguments);
	}else {
		/* ara einai variable*/
